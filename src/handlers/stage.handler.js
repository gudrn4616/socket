...
export const moveStageHandler = (userId, payload) => {
  ...
  if (elapsedTime < 5 || elapsedTime > 105) {
  ...
  return { status: 'success', data: targetStageData };
}

export const specificStageHandler = (userId, payload) => {
  const { stages } = getGameAssets();
  const stage = stages.data.find((stage) => stage.id === payload.currentStage);
  if (stage) {
    return { status: 'success', stage };
  } else {
